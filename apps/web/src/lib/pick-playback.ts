...
export function buildWatchPlayback(
  ...
  if (options?.shorts) {
    ...
    const hasMuxed = merged.some((v) => v.t === "muxed");
    if (hasMuxed) {
    ...
    // No muxed rung: the split path (video-only + separate <audio>) stalls and
    // gets the short skipped. Route to the generated byte-range HLS instead —
    // segmented and reliable, the same path the watch page uses for
    // adaptive-only videos — so these shorts (often full/landscape uploads that
    // only expose adaptive streams) actually play rather than being skipped.
    ...
    // Last resort before HLS/none: play whatever split progressive we have.
    if (merged.length > 0) {
      const variants = buildFullQualitySelectorList(merged);
      return {
        kind: "progressive",
